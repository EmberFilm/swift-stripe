...
(`Stripe`) containing the models, request types and one client per resource — all three
generated from Stripe's OpenAPI specification and checked against it in CI — plus a
hand-written request engine, webhook verification and shared types. Swift 6.3 toolchain,
language mode 6, macOS 26+ / Linux.
...
Tests are hermetic: no network, no Stripe credentials.

CI mirrors the swift-server projects (`.github/workflows/`): `pull_request.yml` runs the
swiftlang **soundness** workflow (license headers, `swift format lint --strict`, unacceptable
language, yamllint, flake8 on `Scripts/`, DocC), the swift-nio **unit tests** (Linux 6.3 and
nightlies with `-require-explicit-sendable`), **static Linux SDK** and **release builds**
reusable workflows, and Vapor's **foundation-linking** check. `main.yml` runs unit tests,
release builds and foundation linking on every push and twice a day. The spec gates —
`Scripts/model-drift.py` and both generators' `--check` — are run locally before committing;
they are not a CI job.
`pull_request_label.yml` requires one semver label per PR, and `auto-release.yml` (Mondays, or
on dispatch) cuts the next release from the labels of PRs merged since the last tag — so changes
land through labelled PRs, not direct pushes; a first or major release is tagged by hand.

Every Swift file starts with the Apache-2.0 header in `.license_header_template` and nothing
else above its imports; the generators emit it, and the soundness job rejects a file without it. `.swift-format` is
swift-temporal-sdk's; the generators run `swift format` on what they write, so generated and
hand-written code are held to the same rules and `--check` compares formatted output.
...
```
Sources/Stripe/
├── Core/        StripeClient, StripeAPI, StripeConfiguration (+ConfigReader),
│                StripeClientError, StripeFormEncoder                     hand-written
├── Webhooks/    StripeWebhook signature verification                     hand-written
├── Shared/      the Stripe namespace, Expandable, Boxed, Clearable,
│                Page/SearchPage/RangeQuery, Upload, Error, Interval      hand-written
├── Models/      one file per resource in a namespace tree               generated (+ retained hand types)
├── Requests/    <Resource>+Requests.swift per resource                   generated (WebElements/ is hand)
└── Clients/     <Resources>Client.swift per resource, StripeClient+Resources  generated
```

Nothing in a generated file marks it as generated; `Scripts/generated-files.txt`, written by the
generators, is what the tooling keys on. Everything else is hand-written.
...
`Requests/Customers/Customer+Requests.swift`, `Clients/CustomersClient.swift`. The file stem is
the type's leaf name, lengthened by parent components only while it would clash — SwiftPM
requires unique basenames across a target, folders notwithstanding — so `Checkout.Session.swift`
but `Customer.swift` (`layout()` in the generator; `model_file()` spells it). A retained hand
file shares the stem: `Checkout/Checkout.Session+Retained.swift`. What marks a file as generated is its
entry in `Scripts/generated-files.txt`, which the generators rewrite on every run; cleanup,
`--check`, the cutover sweep and namespace detection all key on it:
...
`Sources/Stripe/Requests/`. Operations live under the resource's Swift type, named by
...
The same generator emits one client per resource into `Sources/Stripe/Clients/`:
...
CONTRIBUTING.md is for someone sending a change: bug-report shape, the checks to run before
a pull request, the generated-sources rule and the label-driven release process. README.md is
written for someone adopting the package, in the style of the swift-server
libraries: what it is, how to add it, how to use each part, how it is built, license.
