...
swift test                           # run everything (238 tests, 15 suites)
...
Tests are hermetic: no network, no Stripe credentials. CI (`.github/workflows/ci.yml`)
builds and tests on macOS 26 and in the `swift:6.3` Linux container, regenerates against the
pinned spec and fails on any difference (`--check`), runs `Scripts/model-drift.py`, and runs
Vapor's `check-foundation-linking` workflow.
...
Generated files carry the header "Generated by Scripts/…"; that line is what the tooling keys
on. Everything else is hand-written.
...
README.md is written for someone adopting the package, in the style of the swift-server
