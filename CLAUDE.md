...
file shares the stem: `Checkout/Checkout.Session+Retained.swift`. What marks a file as generated
is its header line ("Generated by Scripts/…"); cleanup, `--check`, the cutover sweep and namespace
detection all key on it, so never put that line in a hand-written file:
...
`Sources/Stripe/Requests/`. Operations hang off the resource's Swift type, named by
