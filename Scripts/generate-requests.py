...
class Generator:
    ...
    def resolve(self, node: dict, prop: str, owner: Struct, path: str) -> str:
        ...
        if "anyOf" in node:
            ...
            if types == ["integer", "object"] and set(alts[1 if alts[0].get("type") == "integer" else 0].get("properties", {})) <= {
                    "gt", "gte", "lt", "lte"}:
                return RANGE_QUERY
    ...
    def render_client(self, resource: str, swift_type: str, ops: list[Operation]) -> tuple[str, str]:
        ...
        for op in sorted(ops, key=lambda o: o.name):
            ...
            param_decl = [f"{ident(label)}: {t}" for label, t in labels]
            param_pass = [f"{ident(label)}: {ident(label)}" for label, _ in labels]
            has_request = op.request is not None
            all_optional = has_request and not any(f.required for f in op.request.fields)
            writes = op.http != "get"
            decl = list(param_decl)
            if has_request:
                decl.append(f"_ request: {ns}.Request")
            if multipart:
                decl.append("file: Stripe.Upload")
            if writes:
                decl.append("idempotencyKey: String?")
            ...
            if multipart:
                ...
                conveniences.append(
                    f"    public func {method}({', '.join(param_decl +
                                                          [f'_ request: {ns}.Request', 'file: Stripe.Upload'])}) async throws -> {ns}.Response {{\n"
            ...
            if writes:
                if has_request:
                    ...
                    if all_optional:
                        conveniences.append(
                            f"    public func {method}({', '.join(param_decl +
                                                                  ['idempotencyKey: String? = nil'])}) async throws -> {ns}.Response {{\n"
        ...
        out += f"/// A protocol so tests can substitute a double; ``{name}Client`` is the implementation that\n/// talks to Stripe.\n"
        out += f"public protocol {name}API: Sendable {{\n" + "\n".join(requirements) + "\n}\n\n"
        out += f"public struct {name}Client: {name}API {{\n    private let api: StripeAPI\n\n"
        out += "    public init(api: StripeAPI) { self.api = api }\n\n"
        out += "\n\n".join(bodies) + "\n}\n"
        if conveniences:
            out += "\n// A write with no explicit key behaves as it did before idempotency keys existed: no header,\n"
            out += f"// and no retry. See ``StripeAPI/isSafeToRetry(_:)``.\nextension {name}API {{\n"
            out += "\n\n".join(conveniences) + "\n}\n"
        return name, out
...
def main() -> int:
    ...
    if args.check:
        ...
        for out, files in outputs:
            files = gm.formatted(files)
            stale = [n for n, body in files.items() if not (out / n).exists() or (out / n).read_text() != body]
            extra = [p for p in gm.generated_files(out) if str(p.relative_to(out.resolve())) not in files]
            for n in stale:
    ...
    for out, files in outputs:
        ...
        gm.swift_format([out / n for n in files])
        gm.record_generated(out, files)
