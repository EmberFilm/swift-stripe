//===----------------------------------------------------------------------===//
//
// This source file is part of the swift-stripe open source project
//
// Copyright (c) 2026 the swift-stripe project authors
// Licensed under Apache License v2.0
//
// See LICENSE for license information
// See NOTICE for attribution of derived work
//
// SPDX-License-Identifier: Apache-2.0
//
//===----------------------------------------------------------------------===//

#if canImport(FoundationEssentials)
import FoundationEssentials
#else
import Foundation
#endif

extension Stripe.Fraud {
    /// Payment Evaluations represent the risk lifecycle of an externally processed payment.
    public struct PaymentEvaluation: Codable, Hashable, Sendable, Identifiable {
        public typealias ID = String
        public let id: ID
        /// String representing the object's type.
        public let object: String
        public var clientDeviceMetadataDetails: ClientDeviceMetadataDetails?
        /// Time at which the object was created.
        public var createdAt: Date?
        public var customerDetails: CustomerDetails?
        /// Event information associated with the payment evaluation, such as refunds, dispute, early fraud warnings, or user.
        public var events: [Events]?
        /// If the object exists in live mode, the value is `true`.
        public var livemode: Bool?
        /// Set of key-value pairs that you can attach to an object.
        public var metadata: [String: String]?
        /// Indicates the final outcome for the payment evaluation.
        public var outcome: Outcome?
        public var paymentDetails: PaymentDetails?
        /// Recommended action based on the score of the `fraudulent_payment` signal.
        public var recommendedAction: RecommendedAction?
        public var signals: Signals?

        public init(
            id: ID,
            object: String,
            clientDeviceMetadataDetails: ClientDeviceMetadataDetails? = nil,
            createdAt: Date? = nil,
            customerDetails: CustomerDetails? = nil,
            events: [Events]? = nil,
            livemode: Bool? = nil,
            metadata: [String: String]? = nil,
            outcome: Outcome? = nil,
            paymentDetails: PaymentDetails? = nil,
            recommendedAction: RecommendedAction? = nil,
            signals: Signals? = nil
        ) {
            self.id = id
            self.object = object
            self.clientDeviceMetadataDetails = clientDeviceMetadataDetails
            self.createdAt = createdAt
            self.customerDetails = customerDetails
            self.events = events
            self.livemode = livemode
            self.metadata = metadata
            self.outcome = outcome
            self.paymentDetails = paymentDetails
            self.recommendedAction = recommendedAction
            self.signals = signals
        }

        /// Recommended action based on the score of the `fraudulent_payment` signal.
        public enum RecommendedAction: String, Codable, Hashable, Sendable {
            case block
            case `continue`
        }

        /// Client device metadata attached to this payment evaluation.
        public struct ClientDeviceMetadataDetails: Codable, Hashable, Sendable {
            /// ID for the Radar Session associated with the payment evaluation.
            public var radarSession: String?

            public init(
                radarSession: String? = nil
            ) {
                self.radarSession = radarSession
            }
        }

        /// Customer details attached to this payment evaluation.
        public struct CustomerDetails: Codable, Hashable, Sendable {
            /// The ID of the customer associated with the payment evaluation.
            public var customer: String?
            /// The ID of the Account representing the customer associated with the payment evaluation.
            public var customerAccount: String?
            /// The customer's email address.
            public var email: String?
            /// The customer's full name or business name.
            public var name: String?
            /// The customer's phone number.
            public var phone: String?

            public init(
                customer: String? = nil,
                customerAccount: String? = nil,
                email: String? = nil,
                name: String? = nil,
                phone: String? = nil
            ) {
                self.customer = customer
                self.customerAccount = customerAccount
                self.email = email
                self.name = name
                self.phone = phone
            }
        }

        /// Event reported for this payment evaluation.
        public struct Events: Codable, Hashable, Sendable {
            /// Timestamp when the event occurred.
            public var occurredAt: Date?
            /// Indicates the type of event attached to the payment evaluation.
            public var `type`: Type?
            /// The payload `type` selects.
            public var details: Details

            fileprivate enum CodingKeys: String, CodingKey {
                case occurredAt
                case `type`
                case disputeOpened
                case earlyFraudWarningReceived
                case refunded
                case userInterventionRaised
                case userInterventionResolved
            }

            public init(
                occurredAt: Date? = nil,
                `type`: Type? = nil,
                details: Details
            ) {
                self.occurredAt = occurredAt
                self.`type` = `type`
                self.details = details
            }

            public init(from decoder: any Decoder) throws {
                let container = try decoder.container(keyedBy: CodingKeys.self)
                self.occurredAt = try container.decodeIfPresent(Date.self, forKey: .occurredAt)
                self.`type` = try container.decodeIfPresent(Type.self, forKey: .`type`)
                self.details = try Details(type: try container.decodeIfPresent(String.self, forKey: .type) ?? "", from: container)
            }

            public func encode(to encoder: any Encoder) throws {
                var container = encoder.container(keyedBy: CodingKeys.self)
                try container.encodeIfPresent(occurredAt, forKey: .occurredAt)
                try container.encodeIfPresent(`type`, forKey: .`type`)
                try details.encode(into: &container)
            }

            /// Indicates the type of event attached to the payment evaluation.
            public enum `Type`: String, Codable, Hashable, Sendable {
                case disputeOpened = "dispute_opened"
                case earlyFraudWarningReceived = "early_fraud_warning_received"
                case refunded
                case userInterventionRaised = "user_intervention_raised"
                case userInterventionResolved = "user_intervention_resolved"
            }

            /// Dispute opened event details attached to this payment evaluation.
            public struct DisputeOpened: Codable, Hashable, Sendable {
                /// Amount to dispute for this payment.
                public var amount: Int?
                /// Three-letter ISO currency code, in lowercase.
                public var currency: Stripe.Currency?
                /// Reason given by cardholder for dispute.
                public var reason: Reason?

                public init(
                    amount: Int? = nil,
                    currency: Stripe.Currency? = nil,
                    reason: Reason? = nil
                ) {
                    self.amount = amount
                    self.currency = currency
                    self.reason = reason
                }

                /// Reason given by cardholder for dispute.
                public enum Reason: String, Codable, Hashable, Sendable {
                    case accountNotAvailable = "account_not_available"
                    case creditNotProcessed = "credit_not_processed"
                    case customerInitiated = "customer_initiated"
                    case duplicate
                    case fraudulent
                    case general
                    case noncompliant
                    case productNotReceived = "product_not_received"
                    case productUnacceptable = "product_unacceptable"
                    case subscriptionCanceled = "subscription_canceled"
                    case unrecognized
                }
            }

            /// Early Fraud Warning Received event details attached to this payment evaluation.
            public struct EarlyFraudWarningReceived: Codable, Hashable, Sendable {
                /// The type of fraud labeled by the issuer.
                public var fraudType: FraudType?

                public init(
                    fraudType: FraudType? = nil
                ) {
                    self.fraudType = fraudType
                }

                /// The type of fraud labeled by the issuer.
                public enum FraudType: String, Codable, Hashable, Sendable {
                    case madeWithLostCard = "made_with_lost_card"
                    case madeWithStolenCard = "made_with_stolen_card"
                    case other
                    case unauthorizedUseOfCard = "unauthorized_use_of_card"
                }
            }

            /// Refunded Event details attached to this payment evaluation.
            public struct Refunded: Codable, Hashable, Sendable {
                /// Amount refunded for this payment.
                public var amount: Int?
                /// Three-letter ISO currency code, in lowercase.
                public var currency: Stripe.Currency?
                /// Indicates the reason for the refund.
                public var reason: Reason?

                public init(
                    amount: Int? = nil,
                    currency: Stripe.Currency? = nil,
                    reason: Reason? = nil
                ) {
                    self.amount = amount
                    self.currency = currency
                    self.reason = reason
                }

                /// Indicates the reason for the refund.
                public enum Reason: String, Codable, Hashable, Sendable {
                    case duplicate
                    case fraudulent
                    case other
                    case requestedByCustomer = "requested_by_customer"
                }
            }

            /// User intervention raised event details attached to this payment evaluation.
            public struct UserInterventionRaised: Codable, Hashable, Sendable {
                public var custom: Custom?
                /// Unique identifier for the user intervention event.
                public var key: String?
                /// Type of user intervention raised.
                public var `type`: Type?

                public init(
                    custom: Custom? = nil,
                    key: String? = nil,
                    `type`: Type? = nil
                ) {
                    self.custom = custom
                    self.key = key
                    self.`type` = `type`
                }

                /// Type of user intervention raised.
                public enum `Type`: String, Codable, Hashable, Sendable {
                    case value3ds = "3ds"
                    case captcha
                    case custom
                }

                /// User intervention raised custom event details attached to this payment evaluation.
                public struct Custom: Codable, Hashable, Sendable {
                    /// Custom type of user intervention raised.
                    public var `type`: String?

                    public init(
                        `type`: String? = nil
                    ) {
                        self.`type` = `type`
                    }
                }
            }

            /// User Intervention Resolved Event details attached to this payment evaluation.
            public struct UserInterventionResolved: Codable, Hashable, Sendable {
                /// Unique ID of this intervention.
                public var key: String?
                /// Result of the intervention if it has been completed.
                public var outcome: Outcome?

                public init(
                    key: String? = nil,
                    outcome: Outcome? = nil
                ) {
                    self.key = key
                    self.outcome = outcome
                }

                /// Result of the intervention if it has been completed.
                public enum Outcome: String, Codable, Hashable, Sendable {
                    case abandoned
                    case failed
                    case passed
                }
            }

            /// The payload `type` selects; `unknown` carries a type this package does not model.
            public indirect enum Details: Hashable, Sendable {
                case disputeOpened(DisputeOpened)
                case earlyFraudWarningReceived(EarlyFraudWarningReceived)
                case refunded(Refunded)
                case userInterventionRaised(UserInterventionRaised)
                case userInterventionResolved(UserInterventionResolved)
                case unknown(type: String)

                public var disputeOpened: DisputeOpened? {
                    if case .disputeOpened(let value) = self { return value }
                    return nil
                }
                public var earlyFraudWarningReceived: EarlyFraudWarningReceived? {
                    if case .earlyFraudWarningReceived(let value) = self { return value }
                    return nil
                }
                public var refunded: Refunded? {
                    if case .refunded(let value) = self { return value }
                    return nil
                }
                public var userInterventionRaised: UserInterventionRaised? {
                    if case .userInterventionRaised(let value) = self { return value }
                    return nil
                }
                public var userInterventionResolved: UserInterventionResolved? {
                    if case .userInterventionResolved(let value) = self { return value }
                    return nil
                }

                fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
                    switch type {
                    case "dispute_opened":
                        if let value = try container.decodeIfPresent(DisputeOpened.self, forKey: .disputeOpened) {
                            self = .disputeOpened(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "early_fraud_warning_received":
                        if let value = try container.decodeIfPresent(EarlyFraudWarningReceived.self, forKey: .earlyFraudWarningReceived) {
                            self = .earlyFraudWarningReceived(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "refunded":
                        if let value = try container.decodeIfPresent(Refunded.self, forKey: .refunded) {
                            self = .refunded(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "user_intervention_raised":
                        if let value = try container.decodeIfPresent(UserInterventionRaised.self, forKey: .userInterventionRaised) {
                            self = .userInterventionRaised(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "user_intervention_resolved":
                        if let value = try container.decodeIfPresent(UserInterventionResolved.self, forKey: .userInterventionResolved) {
                            self = .userInterventionResolved(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    default: self = .unknown(type: type)
                    }
                }

                fileprivate func encode(into container: inout KeyedEncodingContainer<CodingKeys>) throws {
                    switch self {
                    case .disputeOpened(let value): try container.encode(value, forKey: .disputeOpened)
                    case .earlyFraudWarningReceived(let value): try container.encode(value, forKey: .earlyFraudWarningReceived)
                    case .refunded(let value): try container.encode(value, forKey: .refunded)
                    case .userInterventionRaised(let value): try container.encode(value, forKey: .userInterventionRaised)
                    case .userInterventionResolved(let value): try container.encode(value, forKey: .userInterventionResolved)
                    default: break
                    }
                }
            }
        }

        /// Outcome details for this payment evaluation.
        public struct Outcome: Codable, Hashable, Sendable {
            /// The PaymentIntent ID associated with the payment evaluation.
            public var paymentIntentId: String?
            /// Indicates the outcome of the payment evaluation.
            public var `type`: Type?
            /// The payload `type` selects.
            public var details: Details

            fileprivate enum CodingKeys: String, CodingKey {
                case paymentIntentId
                case `type`
                case merchantBlocked
                case rejected
                case succeeded
            }

            public init(
                paymentIntentId: String? = nil,
                `type`: Type? = nil,
                details: Details
            ) {
                self.paymentIntentId = paymentIntentId
                self.`type` = `type`
                self.details = details
            }

            public init(from decoder: any Decoder) throws {
                let container = try decoder.container(keyedBy: CodingKeys.self)
                self.paymentIntentId = try container.decodeIfPresent(String.self, forKey: .paymentIntentId)
                self.`type` = try container.decodeIfPresent(Type.self, forKey: .`type`)
                self.details = try Details(type: try container.decodeIfPresent(String.self, forKey: .type) ?? "", from: container)
            }

            public func encode(to encoder: any Encoder) throws {
                var container = encoder.container(keyedBy: CodingKeys.self)
                try container.encodeIfPresent(paymentIntentId, forKey: .paymentIntentId)
                try container.encodeIfPresent(`type`, forKey: .`type`)
                try details.encode(into: &container)
            }

            /// Indicates the outcome of the payment evaluation.
            public enum `Type`: String, Codable, Hashable, Sendable {
                case failed
                case merchantBlocked = "merchant_blocked"
                case rejected
                case succeeded
            }

            /// Details of a merchant_blocked outcome attached to this payment evaluation.
            public struct MerchantBlocked: Codable, Hashable, Sendable {
                /// The reason the payment was blocked by the merchant.
                public var reason: Reason?

                public init(
                    reason: Reason? = nil
                ) {
                    self.reason = reason
                }

                /// The reason the payment was blocked by the merchant.
                public enum Reason: String, Codable, Hashable, Sendable {
                    case authenticationRequired = "authentication_required"
                    case blockedForFraud = "blocked_for_fraud"
                    case invalidPayment = "invalid_payment"
                    case other
                }
            }

            /// Details of an rejected outcome attached to this payment evaluation.
            public struct Rejected: Codable, Hashable, Sendable {
                @Boxed public var card: Card?

                public init(
                    card: Card? = nil
                ) {
                    self._card = Boxed(wrappedValue: card)
                }

                /// Details of an rejected card outcome attached to this payment evaluation.
                public struct Card: Codable, Hashable, Sendable {
                    /// Result of the address line 1 check.
                    public var addressLine1Check: AddressLine1Check?
                    /// Indicates whether the cardholder provided a postal code and if it matched the cardholder’s billing address.
                    public var addressPostalCodeCheck: AddressPostalCodeCheck?
                    /// Result of the CVC check.
                    public var cvcCheck: CvcCheck?
                    /// Card issuer's reason for the network decline.
                    public var reason: Reason?

                    public init(
                        addressLine1Check: AddressLine1Check? = nil,
                        addressPostalCodeCheck: AddressPostalCodeCheck? = nil,
                        cvcCheck: CvcCheck? = nil,
                        reason: Reason? = nil
                    ) {
                        self.addressLine1Check = addressLine1Check
                        self.addressPostalCodeCheck = addressPostalCodeCheck
                        self.cvcCheck = cvcCheck
                        self.reason = reason
                    }

                    /// Result of the address line 1 check.
                    public enum AddressLine1Check: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }

                    /// Indicates whether the cardholder provided a postal code and if it matched the cardholder’s billing address.
                    public enum AddressPostalCodeCheck: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }

                    /// Result of the CVC check.
                    public enum CvcCheck: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }

                    /// Card issuer's reason for the network decline.
                    public enum Reason: String, Codable, Hashable, Sendable {
                        case authenticationFailed = "authentication_failed"
                        case doNotHonor = "do_not_honor"
                        case expired
                        case incorrectCvc = "incorrect_cvc"
                        case incorrectNumber = "incorrect_number"
                        case incorrectPostalCode = "incorrect_postal_code"
                        case insufficientFunds = "insufficient_funds"
                        case invalidAccount = "invalid_account"
                        case lostCard = "lost_card"
                        case other
                        case processingError = "processing_error"
                        case reportedStolen = "reported_stolen"
                        case tryAgainLater = "try_again_later"
                    }
                }
            }

            /// Details of a succeeded outcome attached to this payment evaluation.
            public struct Succeeded: Codable, Hashable, Sendable {
                @Boxed public var card: Card?

                public init(
                    card: Card? = nil
                ) {
                    self._card = Boxed(wrappedValue: card)
                }

                /// Details of an succeeded card outcome attached to this payment evaluation.
                public struct Card: Codable, Hashable, Sendable {
                    /// Result of the address line 1 check.
                    public var addressLine1Check: AddressLine1Check?
                    /// Indicates whether the cardholder provided a postal code and if it matched the cardholder’s billing address.
                    public var addressPostalCodeCheck: AddressPostalCodeCheck?
                    /// Result of the CVC check.
                    public var cvcCheck: CvcCheck?

                    public init(
                        addressLine1Check: AddressLine1Check? = nil,
                        addressPostalCodeCheck: AddressPostalCodeCheck? = nil,
                        cvcCheck: CvcCheck? = nil
                    ) {
                        self.addressLine1Check = addressLine1Check
                        self.addressPostalCodeCheck = addressPostalCodeCheck
                        self.cvcCheck = cvcCheck
                    }

                    /// Result of the address line 1 check.
                    public enum AddressLine1Check: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }

                    /// Indicates whether the cardholder provided a postal code and if it matched the cardholder’s billing address.
                    public enum AddressPostalCodeCheck: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }

                    /// Result of the CVC check.
                    public enum CvcCheck: String, Codable, Hashable, Sendable {
                        case fail
                        case pass
                        case unavailable
                        case unchecked
                    }
                }
            }

            /// The payload `type` selects; `unknown` carries a type this package does not model.
            public indirect enum Details: Hashable, Sendable {
                case merchantBlocked(MerchantBlocked)
                case rejected(Rejected)
                case succeeded(Succeeded)
                case failed
                case unknown(type: String)

                public var merchantBlocked: MerchantBlocked? {
                    if case .merchantBlocked(let value) = self { return value }
                    return nil
                }
                public var rejected: Rejected? {
                    if case .rejected(let value) = self { return value }
                    return nil
                }
                public var succeeded: Succeeded? {
                    if case .succeeded(let value) = self { return value }
                    return nil
                }

                fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
                    switch type {
                    case "merchant_blocked":
                        if let value = try container.decodeIfPresent(MerchantBlocked.self, forKey: .merchantBlocked) {
                            self = .merchantBlocked(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "rejected":
                        if let value = try container.decodeIfPresent(Rejected.self, forKey: .rejected) {
                            self = .rejected(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "succeeded":
                        if let value = try container.decodeIfPresent(Succeeded.self, forKey: .succeeded) {
                            self = .succeeded(value)
                        } else {
                            self = .unknown(type: type)
                        }
                    case "failed": self = .failed
                    default: self = .unknown(type: type)
                    }
                }

                fileprivate func encode(into container: inout KeyedEncodingContainer<CodingKeys>) throws {
                    switch self {
                    case .merchantBlocked(let value): try container.encode(value, forKey: .merchantBlocked)
                    case .rejected(let value): try container.encode(value, forKey: .rejected)
                    case .succeeded(let value): try container.encode(value, forKey: .succeeded)
                    default: break
                    }
                }
            }
        }

        /// Payment details attached to this payment evaluation.
        public struct PaymentDetails: Codable, Hashable, Sendable {
            /// Amount intended to be collected by this payment.
            public var amount: Int?
            /// Three-letter ISO currency code, in lowercase.
            public var currency: Stripe.Currency?
            /// An arbitrary string attached to the object.
            public var description: String?
            /// Details about the payment's customer presence and type.
            public var moneyMovementDetails: MoneyMovementDetails?
            /// Details about the payment method used for the payment.
            public var paymentMethodDetails: PaymentMethodDetails?
            /// Shipping details for the payment evaluation.
            public var shippingDetails: ShippingDetails?
            /// Payment statement descriptor.
            public var statementDescriptor: String?

            public init(
                amount: Int? = nil,
                currency: Stripe.Currency? = nil,
                description: String? = nil,
                moneyMovementDetails: MoneyMovementDetails? = nil,
                paymentMethodDetails: PaymentMethodDetails? = nil,
                shippingDetails: ShippingDetails? = nil,
                statementDescriptor: String? = nil
            ) {
                self.amount = amount
                self.currency = currency
                self.description = description
                self.moneyMovementDetails = moneyMovementDetails
                self.paymentMethodDetails = paymentMethodDetails
                self.shippingDetails = shippingDetails
                self.statementDescriptor = statementDescriptor
            }

            /// Money Movement details attached to this payment.
            public struct MoneyMovementDetails: Codable, Hashable, Sendable {
                /// Describes card money movement details for the payment evaluation.
                @Boxed public var card: Card?
                /// Describes the type of money movement.
                public var moneyMovementType: String?

                public init(
                    card: Card? = nil,
                    moneyMovementType: String? = nil
                ) {
                    self._card = Boxed(wrappedValue: card)
                    self.moneyMovementType = moneyMovementType
                }

                /// Money Movement card details attached to this payment.
                public struct Card: Codable, Hashable, Sendable {
                    /// Describes the presence of the customer during the payment.
                    public var customerPresence: CustomerPresence?
                    /// Describes the type of payment.
                    public var paymentType: PaymentType?

                    public init(
                        customerPresence: CustomerPresence? = nil,
                        paymentType: PaymentType? = nil
                    ) {
                        self.customerPresence = customerPresence
                        self.paymentType = paymentType
                    }

                    /// Describes the presence of the customer during the payment.
                    public enum CustomerPresence: String, Codable, Hashable, Sendable {
                        case offSession = "off_session"
                        case onSession = "on_session"
                    }

                    /// Describes the type of payment.
                    public enum PaymentType: String, Codable, Hashable, Sendable {
                        case oneOff = "one_off"
                        case recurring
                        case setupOneOff = "setup_one_off"
                        case setupRecurring = "setup_recurring"
                    }
                }
            }

            /// Payment method details attached to this payment evaluation.
            public struct PaymentMethodDetails: Codable, Hashable, Sendable {
                /// Billing information associated with the payment evaluation.
                public var billingDetails: BillingDetails?
                /// The payment method used in this payment evaluation.
                @Expandable<Stripe.PaymentMethods.PaymentMethod, String> public var paymentMethod: String?

                public init(
                    billingDetails: BillingDetails? = nil,
                    paymentMethod: String? = nil
                ) {
                    self.billingDetails = billingDetails
                    self._paymentMethod = Expandable(id: paymentMethod)
                }

                /// Billing details attached to this payment evaluation.
                public struct BillingDetails: Codable, Hashable, Sendable {
                    public var address: Address?
                    /// Email address.
                    public var email: String?
                    /// Full name.
                    public var name: String?
                    /// Billing phone number (including extension).
                    public var phone: String?

                    public init(
                        address: Address? = nil,
                        email: String? = nil,
                        name: String? = nil,
                        phone: String? = nil
                    ) {
                        self.address = address
                        self.email = email
                        self.name = name
                        self.phone = phone
                    }

                    /// Address data.
                    public struct Address: Codable, Hashable, Sendable {
                        /// City, district, suburb, town, or village.
                        public var city: String?
                        /// Two-letter country code (ISO 3166-1 alpha-2).
                        public var country: String?
                        /// Address line 1, such as the street, PO Box, or company name.
                        public var line1: String?
                        /// Address line 2, such as the apartment, suite, unit, or building.
                        public var line2: String?
                        /// ZIP or postal code.
                        public var postalCode: String?
                        /// State, county, province, or region (ISO 3166-2).
                        public var state: String?

                        public init(
                            city: String? = nil,
                            country: String? = nil,
                            line1: String? = nil,
                            line2: String? = nil,
                            postalCode: String? = nil,
                            state: String? = nil
                        ) {
                            self.city = city
                            self.country = country
                            self.line1 = line1
                            self.line2 = line2
                            self.postalCode = postalCode
                            self.state = state
                        }
                    }
                }
            }

            /// Shipping details attached to this payment.
            public struct ShippingDetails: Codable, Hashable, Sendable {
                public var address: Address?
                /// Shipping name.
                public var name: String?
                /// Shipping phone number.
                public var phone: String?

                public init(
                    address: Address? = nil,
                    name: String? = nil,
                    phone: String? = nil
                ) {
                    self.address = address
                    self.name = name
                    self.phone = phone
                }

                /// Address data.
                public struct Address: Codable, Hashable, Sendable {
                    /// City, district, suburb, town, or village.
                    public var city: String?
                    /// Two-letter country code (ISO 3166-1 alpha-2).
                    public var country: String?
                    /// Address line 1, such as the street, PO Box, or company name.
                    public var line1: String?
                    /// Address line 2, such as the apartment, suite, unit, or building.
                    public var line2: String?
                    /// ZIP or postal code.
                    public var postalCode: String?
                    /// State, county, province, or region (ISO 3166-2).
                    public var state: String?

                    public init(
                        city: String? = nil,
                        country: String? = nil,
                        line1: String? = nil,
                        line2: String? = nil,
                        postalCode: String? = nil,
                        state: String? = nil
                    ) {
                        self.city = city
                        self.country = country
                        self.line1 = line1
                        self.line2 = line2
                        self.postalCode = postalCode
                        self.state = state
                    }
                }
            }
        }

        /// Collection of signals for this payment evaluation.
        public struct Signals: Codable, Hashable, Sendable {
            public var fraudulentPayment: FraudulentPayment?

            public init(
                fraudulentPayment: FraudulentPayment? = nil
            ) {
                self.fraudulentPayment = fraudulentPayment
            }

            /// A payment evaluation signal with evaluated_at, risk_level, and score fields.
            public struct FraudulentPayment: Codable, Hashable, Sendable {
                /// The time when this signal was evaluated.
                public var evaluatedAt: Date?
                /// Risk level of this signal, based on the score.
                public var riskLevel: RiskLevel?
                /// Score for this signal.
                public var score: Double?

                public init(
                    evaluatedAt: Date? = nil,
                    riskLevel: RiskLevel? = nil,
                    score: Double? = nil
                ) {
                    self.evaluatedAt = evaluatedAt
                    self.riskLevel = riskLevel
                    self.score = score
                }

                /// Risk level of this signal, based on the score.
                public enum RiskLevel: String, Codable, Hashable, Sendable {
                    case elevated
                    case highest
                    case low
                    case normal
                    case notAssessed = "not_assessed"
                    case unknown
                }
            }
        }
    }
}
