...
extension Stripe.Charges {
    ...
    public struct Charge: Codable, Hashable, Sendable, Identifiable {
        ...
        /// Amount in cents (or local equivalent) refunded (can be less than the amount attribute on the charge if a partial refun…
        ...
        /// The full statement descriptor that is passed to card networks, and that is displayed on your customers' credit card an…
        ...
        /// ID of the transfer to the `destination` account (only applicable if the charge was created using the `destination` par…
        ...
        public struct Outcome: Codable, Hashable, Sendable {
            ...
            /// For charges declined by the network, a 2 digit code which indicates the advice returned by the network on how to proce…
            ...
            /// A human-readable description of the outcome type and reason, designed for you (the recipient of the payment), not your…
            ...
            public struct Card: Codable, Hashable, Sendable {
                ...
                public struct Wallet: Codable, Hashable, Sendable {
                    ...
                    public indirect enum Details: Hashable, Sendable {
                        ...
                        public var amexExpressCheckout: AmexExpressCheckout? { if case .amexExpressCheckout(let value) = self { return value }; return nil }
                        public var applePay: Stripe.Shared.ApplePay? { if case .applePay(let value) = self { return value }; return nil }
                        public var googlePay: Stripe.Shared.GooglePay? { if case .googlePay(let value) = self { return value }; return nil }
                        public var link: Link? { if case .link(let value) = self { return value }; return nil }
                        public var masterpass: Masterpass? { if case .masterpass(let value) = self { return value }; return nil }
                        public var samsungPay: SamsungPay? { if case .samsungPay(let value) = self { return value }; return nil }
                        public var visaCheckout: VisaCheckout? { if case .visaCheckout(let value) = self { return value }; return nil }

                        fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
                            switch type {
                            case "amex_express_checkout":
                                if let value = try container.decodeIfPresent(AmexExpressCheckout.self, forKey: .amexExpressCheckout) { self = .amexExpressCheckout(value) } else { self = .unknown(type: type) }
                            case "apple_pay":
                                if let value = try container.decodeIfPresent(Stripe.Shared.ApplePay.self, forKey: .applePay) { self = .applePay(value) } else { self = .unknown(type: type) }
                            case "google_pay":
                                if let value = try container.decodeIfPresent(Stripe.Shared.GooglePay.self, forKey: .googlePay) { self = .googlePay(value) } else { self = .unknown(type: type) }
                            case "link":
                                if let value = try container.decodeIfPresent(Link.self, forKey: .link) { self = .link(value) } else { self = .unknown(type: type) }
                            case "masterpass":
                                if let value = try container.decodeIfPresent(Masterpass.self, forKey: .masterpass) { self = .masterpass(value) } else { self = .unknown(type: type) }
                            case "samsung_pay":
                            ...
                            case "visa_checkout":
                                if let value = try container.decodeIfPresent(VisaCheckout.self, forKey: .visaCheckout) { self = .visaCheckout(value) } else { self = .unknown(type: type) }
