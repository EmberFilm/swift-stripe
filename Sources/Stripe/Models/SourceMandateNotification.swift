...
/// Source mandate notifications should be created when a notification related to a source mandate must be sent to the.
public struct SourceMandateNotification: Codable, Hashable, Sendable, Identifiable {
    ...
    public indirect enum Details: Hashable, Sendable {
        ...
        fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
            ...
            case "source":
                if let value = try container.decodeIfPresent(Source.self, forKey: .source) {
                    self = .source(value)
                } else {
                    self = .unknown(type: type)
                }
