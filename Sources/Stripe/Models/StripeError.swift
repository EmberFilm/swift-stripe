...
/// etc.). Codes in the `5xx` range indicate an error with Stripe's servers (these are rare). Some `4xx`
/// errors that could be handled programmatically (e.g., a card is declined) include an error code that
/// briefly explains the error reported.
public final class StripeError: Codable, Hashable, Swift.Error {
    public let error: StripeError.Error?
...
extension StripeError {
    public final class Error: Codable, Hashable, Sendable {
        /// The type of error returned.
        ///
        /// One of `api_connection_error`, `api_error`, `authentication_error`, `card_error`,
        /// `idempotency_error`, `invalid_request_error`, or `rate_limit_error`.
        public let type: StripeError.Variant?
        /// For card errors, the ID of the failed charge.
        public let charge: Stripe.Charges.Charge.ID?
        /// For some errors that could be handled programmatically, a short string indicating the error code reported.
        public let code: StripeErrorCode?
        /// For card errors resulting from a card issuer decline, a short string indicating the [card issuer’s reason for the decline](https://stripe.com/docs/declines#issuer-declines) if they provide one.
        public let declineCode: StripeDeclineCode?
        ...
        public let paymentIntent: Stripe.PaymentIntents.PaymentIntent?
        ...
        public init(
            ...
            declineCode: StripeDeclineCode? = nil,
            docUrl: String? = nil,
            message: String? = nil,
            param: String? = nil,
        ...
        public func hash(into hasher: inout Hasher) {
            ...
            hasher.combine(code)
            hasher.combine(declineCode)
            hasher.combine(docUrl)
            hasher.combine(message)
            hasher.combine(param)
            hasher.combine(paymentIntent)
            hasher.combine(paymentMethod)
            hasher.combine(source)
        }

        public static func == (lhs: StripeError.Error, rhs: StripeError.Error) -> Bool {
            lhs.type == rhs.type && lhs.charge == rhs.charge && lhs.code == rhs.code
                && lhs.declineCode == rhs.declineCode && lhs.docUrl == rhs.docUrl
                && lhs.message == rhs.message && lhs.param == rhs.param
                && lhs.paymentIntent == rhs.paymentIntent && lhs.paymentMethod == rhs.paymentMethod
...
public enum StripeErrorCode: String, Codable, Sendable {
    /// The email address provided for the creation of a deferred account already has an account associated with it.
    ///
    ...
    ///
    /// Bank account information varies from country to country. We recommend creating validations in
    /// your entry forms based on the bank account formats we provide.
    ...
    /// The latest authentication attempt for the AuthenticationIntent has failed.
    ///
    /// Check the `last_authentication_error` property on the AuthenticationIntent for more details, and
    /// provide a new payment method to attempt to authenticate again.
    ...
    ///
    /// Create a new transfer or payout using an amount less than or equal to the account’s available
    /// balance.
    case bankAccountExists = "bank_account_exists"
    /// The bank account provided already exists on the specified Customer object.
    ///
    /// If the bank account should also be attached to a different customer, include the correct
    /// customer ID when making the request again.
    case bankAccountUnusable = "bank_account_unsuable"
    /// The bank account provided cannot be used for payouts.
    ///
    /// A different bank account must be used.
    ...
    ///
    /// When a card is declined, the error returned also includes the decline_code attribute with the
    /// reason why the card was declined. Refer to our decline codes documentation to learn more.
    ...
    ///
    /// Auth and capture charges must be captured within seven days.
    ...
    ///
    /// Check the expiration date or use a different card.
    ...
    /// The card’s address is incorrect.
    ///
    /// Check the card’s address or use a different card.
    ...
    ///
    /// Check the card’s number or use a different card.
    case incorrectNumber = "incorrect_number"
    /// The card’s ZIP code is incorrect.
    ///
    /// Check the card’s ZIP code or use a different card.
    case incorrectZip = "incorrect_zip"
    /// The debit card provided as an external account does not support instant payouts.
    ///
    /// Provide another debit card or use a bank account instead.
    case instantPayoutUnsupported = "instant_payouts_unsupported"
    /// The card provided as an external account is not a debit card.
    ///
    /// Provide a debit card or use a bank account instead.
    case invalidCardType = "invalid_card_type"
    /// The specified amount is invalid.
    ///
    /// The charge amount must be a positive integer in the smallest currency unit, and not exceed the
    ...
    /// The card’s expiration month is incorrect.
    ///
    /// Check the expiration date or use a different card.
    case invalidExpiryMonth = "invalid_expiry_month"
    /// The card’s expiration year is incorrect.
    ///
    /// Check the expiration date or use a different card.
    case invalidExpiryYear = "invalid_expiry_year"
    /// The card number is invalid.
    ///
    /// Check the card details or use a different card.
    ...
    ///
    /// Check the status of the source you are attempting to use.
    case invalidSourceUsage = "invalid_source_usage"
    /// An invoice cannot be generated for the specified customer as there are no pending invoice items.
    ///
    ...
    /// An invoice cannot be generated for the specified subscription as there are no pending invoice items.
    ///
    /// Check that the correct subscription is being specified or create any necessary invoice items
    /// first.
    case invoiceNoSubscriptionLineItems = "invoice_no_subscription_line_items"
    /// The specified invoice can no longer be edited.
    ///
    /// Instead, consider creating additional invoice items that will be applied to the next invoice.
    /// You can either manually generate the next invoice or wait for it to be automatically generated
    /// at the end of the billing cycle.
    case invoiceNotEditable = "invoice_not_editable"
    /// There is no upcoming invoice on the specified customer to preview.
    ///
    /// Only customers with active subscriptions or pending invoice items have invoices that can be
    /// previewed.
    ...
    /// Both a customer and source ID have been provided, but the source has not been saved to the customer.
    ///
    ...
    ///
    /// Check the order details and then try again.
    ...
    ///
    /// If more stock is available, update the SKU’s inventory quantity and try again.
    case outOfInventory = "out_of_inventory"
    /// One or more required values were not provided.
    ///
    /// Make sure requests include all required parameters.
    ...
    ///
    /// Make sure that only supported values are provided for each attribute. Refer to our API
    /// documentation to look up the type of data each attribute supports.
    ...
    ///
    /// Check our API documentation to see which values are required to create or modify the specified
    /// resource.
    ...
    ///
    /// Check our API documentation or the returned error message to see which values are permitted when
    /// creating or modifying the specified resource.
    ...
    /// One or more provided parameters was not allowed for the given operation on the PaymentIntent.
    ///
    /// Check our API reference or the returned error message to see which values were not correct for
    /// that PaymentIntent.
    ...
    ///
    /// Check the `last_payment_error` property on the PaymentIntent for more details, and provide
    /// source_data or a new source to attempt to fulfill this PaymentIntent again.
    ...
    /// The charge cannot be created as the payment method used has not been activated.
    ///
    ...
    ///
    /// Confirm that the payment method is in an allowed state for the given operation before attempting
    /// to perform it.
    case paymentMethodUnexpectedState = "payment_method_unexpected_state"
    /// Payouts have been disabled on the connected account.
    ///
    /// Check the connected account’s status to see if any additional information needs to be provided,
    /// or if payouts have been disabled for another reason.
    case payoutsNotAllowed = "payouts_not_allowed"
    /// The API key provided by your Connect platform has expired.
    ///
    /// This occurs if your platform has either generated a new key or the connected account has been
    /// disconnected from the platform. Obtain your current API keys from the Dashboard and update your
    ...
    ///
    /// Check the card details are correct or use a different card.
    ...
    ///
    /// Either the resource does not exist, or an ID for a different resource has been provided.
    ...
    /// The SKU is inactive and no longer available for purchase.
    ///
    /// Use a different SKU, or make the current SKU active again.
    case skuInactive = "sku_inactive"
    /// Occurs when providing the `legal_entity` information for a U.S. custom account, if the provided state is not supported.
    ///
    ...
    /// The token provided has already been used.
    ///
    /// You must create a new token before you can retry this request.
    ...
    /// The requested transfer cannot be created.
    ///
    /// Contact us if you are receiving this error.
...
public enum StripeDeclineCode: String, Codable, Sendable {
    ...
    ///
    /// This decline code only applies to payments made with a card reader.
    ...
    /// The request was in test mode, but used a non test (live) card.
    ///
    /// For a list of valid test cards, visit: https://stripe.com/docs/testing.
