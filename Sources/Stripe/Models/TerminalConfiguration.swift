    ...
    public struct Wifi: Codable, Hashable, Sendable {
        ...
        public indirect enum Details: Hashable, Sendable {
            ...
            public var enterpriseEapPeap: EnterpriseEapPeap? {
                if case .enterpriseEapPeap(let value) = self { return value }
                return nil
            }
            public var enterpriseEapTls: EnterpriseEapTls? {
                if case .enterpriseEapTls(let value) = self { return value }
                return nil
            }
            public var personalPsk: PersonalPsk? {
                if case .personalPsk(let value) = self { return value }
                return nil
            }
            ...
            fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
                ...
                case "enterprise_eap_peap":
                    if let value = try container.decodeIfPresent(EnterpriseEapPeap.self, forKey: .enterpriseEapPeap) {
                        self = .enterpriseEapPeap(value)
                    } else {
                        self = .unknown(type: type)
                    }
                case "enterprise_eap_tls":
                    if let value = try container.decodeIfPresent(EnterpriseEapTls.self, forKey: .enterpriseEapTls) {
                        self = .enterpriseEapTls(value)
                    } else {
                        self = .unknown(type: type)
                    }
                case "personal_psk":
                    if let value = try container.decodeIfPresent(PersonalPsk.self, forKey: .personalPsk) {
                        self = .personalPsk(value)
                    } else {
                        self = .unknown(type: type)
                    }
