...
public struct Source: Codable, Hashable, Sendable, Identifiable {
    ...
    public struct CodeVerification: Codable, Hashable, Sendable {
        ...
        /// The status of the code verification, either `pending` (awaiting verification, `attempts_remaining` should be greater.
        public var status: String?
    ...
    public indirect enum Details: Hashable, Sendable {
        ...
        public var threeDSecure: ThreeDSecure? {
            if case .threeDSecure(let value) = self { return value }
            return nil
        }
        ...
        fileprivate init(type: String, from container: KeyedDecodingContainer<CodingKeys>) throws {
            ...
            case "ach_credit_transfer":
                if let value = try container.decodeIfPresent(AchCreditTransfer.self, forKey: .achCreditTransfer) {
                    self = .achCreditTransfer(value)
                } else {
                    self = .unknown(type: type)
                }
            case "ach_debit":
                if let value = try container.decodeIfPresent(AchDebit.self, forKey: .achDebit) {
                    self = .achDebit(value)
                } else {
                    self = .unknown(type: type)
                }
            ...
            case "card_present":
                if let value = try container.decodeIfPresent(CardPresent.self, forKey: .cardPresent) {
            ...
            case "sepa_credit_transfer":
                if let value = try container.decodeIfPresent(SepaCreditTransfer.self, forKey: .sepaCreditTransfer) {
            ...
            case "three_d_secure":
                if let value = try container.decodeIfPresent(ThreeDSecure.self, forKey: .threeDSecure) {
                    self = .threeDSecure(value)
                } else {
                    self = .unknown(type: type)
                }
            case "wechat":
                if let value = try container.decodeIfPresent(Wechat.self, forKey: .wechat) {
                    self = .wechat(value)
                } else {
                    self = .unknown(type: type)
                }
