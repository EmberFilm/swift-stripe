...
struct UnionDecodingTests {
    ...
    func detailsCase() throws {
        ...
        guard case .card(let card) = details else { Issue.record("expected .card, got \(details)"); return }
    ...
    func detailsUnknown() throws {
        ...
        let bare = try Self.decode(Stripe.Charges.Charge.self, """
    ...
    func rootUnion() throws {
        let method = try Self.decode(Stripe.PaymentMethods.PaymentMethod.self, """
    ...
    func paymentSource() throws {
        let sources = try Self.decode([Stripe.PaymentSource].self, """
